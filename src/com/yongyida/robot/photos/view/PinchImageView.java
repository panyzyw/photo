package com.yongyida.robot.photos.view;
...
public class PinchImageView extends ImageView {
    ...
    private boolean isLeftRange = false;
    private boolean isRightRange = false;
    
    private boolean isCanLeft = false;
    private boolean isCanRight = false;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
        	...
        	float dx = event.getX() - mLastMovePoint.x;
        	
//        	if(rect.width()<(float)getWidth()+0.01||rect.height()<(float)getHeight()+0.01){
//        		isViewpager(true);
//			}else{
//		    	if(isCanRight && dx < 0){
//		    		//("继续往右滑动", "isViewpager(true);");
//		    		isViewpager(true);
//		    		isCanRight = false;
//				}else if(isCanLeft && dx > 0){
//					//("继续往左滑动", "isViewpager(true);");
//					isViewpager(true);
//					isCanLeft = false;
//				}else{
//					isViewpager(false);
...
//		    }
	...
	private void setSliding(RectF rect) {
		if(rect.width()<(float)getWidth()+0.01||rect.height()<(float)getHeight()+0.01){
			isViewpager(true);
		}else{
//				if(isRightRange){
...
//					isCanLeft = false;
//					isRightRange = false;
//					//("第二次按下右边到边", "如果继续往右滑动则isViewpager(true);");
//				}else if(isLeftRange){
//					isViewpager(true);
//					isCanLeft = true;
//					isCanRight = false;
//					isLeftRange = false;
//					//("第二次按下左边到边", "如果继续往左滑动则isViewpager(true);");
//				}else{
//					isViewpager(false);
//				}	
			isViewpager(false);
...
//                //右边已经移动到了边界", "已经移动到了边界"
	...
	private void isViewpager(boolean isChange) {
		if(isChange){
			if(getParent().getParent() instanceof ViewPager){
